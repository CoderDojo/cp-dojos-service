...
lab.experiment('submitting lead', function () {
  var plugin = 'cd-dojos';
  var user = {id: 1000, name: 'champ1', email: 'champion1@example.com', roles: ['basic-user']};
  lab.beforeEach(function (done) {
    seneca.make$('cd/dojoleads').remove$({all$: true});
    seneca.make$('cd/dojos').remove$({all$: true});
    done();
  });
  lab.experiment('as a future champion to review', function () {
    var dojo;
    var sent = false;
  ...
  lab.experiment('as a CDF', function () {
    var dojo;
    var sent = false;
    lab.before(function (done) {
      mockSeneca({role: 'cd-profiles', cmd: 'load_user_profile'}, user);
      ...
      seneca.act({role: plugin, ctrl: 'lead', cmd: 'save', lead: leadStub, user: user}, function (err, savedLead) {
        ...
        seneca.act({role: plugin, ctrl: 'dojo', cmd: 'submit', dojo: dojo, user: {id: 1, name: 'john cena', email: 'pet@cd.org', roles: ['cdf-admin']}},
          function (err, submittedDojo) {
            dojo = submittedDojo;
            done();
        });
    ...
    lab.test('CDF admin should be not be notified', function (done) {
