...
module.exports = function cmd_delete (args, done) {
  ...
  function deleteDojo (wfCb) {
    ...
    seneca.act({role: 'cd-dojos', entity: 'dojo', cmd: 'save', dojo: saveDojo}, wfCb);
  }

  function deleteUsersDojos (dojo, wfCb) {
    seneca.act({role: 'cd-dojos', entity: 'userdojo', cmd: 'list', query: {dojoId: dojo.id}}, function (err, list) {
      if (err) return done(err);
      console.log('tobedeleted', list.length);
      if (list && list.length > 0) {
        async.each(list, function (ent, cb) {
          ent.deleted = 1;
          ent.deletedBy = user.id;
          ent.deletedAt = new Date();
          seneca.act({role: 'cd-dojos', entity: 'userdojo', cmd: 'save', userdojo: ent}, cb);
        }, wfCb);
      } else {
        wfCb();
      }
  ...
  function deleteDojoLead (wfCb) {
    ...
    var ent = {};
    ent.id = dojo.dojoLeadId;
    ent.completed = true;
    ent.deleted = 1;
    ent.deletedBy = user.id;
    ent.deletedAt = new Date();
    seneca.act({role: plugin, entity: 'lead', cmd: 'save', lead: ent}, wfCb);
  }
};
