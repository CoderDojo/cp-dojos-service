...
lab.experiment('Lead/Save', function () {
  ...
  lab.experiment('saveLead', function () {
    ...
    lab.test('should save a lead with original lead info, not submitter ', function (done) {
      mockSeneca({role: 'cd-profiles', cmd: 'load_user_profile'}, user);
      mockSeneca({role: 'cd-agreements', cmd: 'getVersion'}, {version: 2});
      seneca.act({role: plugin, ctrl: 'lead', cmd: 'save', lead: leadStub, user: {id: 2000, email: 'notrightemail'}}, function (err, lead) {
        if (err) return done(err);
        expect(lead.userId).to.equal(leadStub.userId);
