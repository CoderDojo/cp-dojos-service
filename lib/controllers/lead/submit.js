...
module.exports = function (args, done) {
  ...
  async.waterfall([
    function (wCb) {
      ...
      lead.completed = true;
      seneca.act({role: 'cd-dojos', entity: 'lead', cmd: 'save', lead: lead, user: args.user}, function (err, lead) {
    ...
    function (wCb) {
      ...
      seneca.act({role: 'cd-dojos', ctrl: 'dojo', cmd: 'submit', dojo: dojo, user: args.user}, function (err, dojo) {
        if (err) return done(err);
