...
module.exports.v1 = function (args, done) {
  var seneca = this;
  ...
  function migrateDojoListing (lead, newLead, wfCb) {
    ...
    if (dojoListing) {
      ...
      dojo.twitter = dojo.twitter ? dojo.twitter.replace('@', '') : '';
  ...
  function migrateChampionDetails (lead, newLead, wfCb) {
    ...
    seneca.act({role: 'cd-profiles', cmd: 'load_user_profile', userId: lead.userId}, function (err, user) {
      ...
      if (championDetails) {
        ...
        champion.twitter = champion.twitter ? champion.twitter.replace('@', '') : '';
...
// Remove extraneous fields (not accepted by API anymore) from old converted leads
function removeExtraField (lead, eCb, wfCb) {
  var toBeSaved = false;
  if (lead.application.venue) {
    if (lead.application.venue.country && lead.application.venue.country.$$hashKey) {
      delete lead.application.venue.country.$$hashKey;
      toBeSaved = true;
    }
    if (lead.application.venue.place && lead.application.venue.place.$$hashKey) {
      delete lead.application.venue.place.$$hashKey;
      toBeSaved = true;
    }
    if (toBeSaved) return wfCb(null, lead, eCb);
  }
  // eCb is coming from the eachSeries. It allows us to bypass the save action for non-modified leads
  return eCb(null, lead);
}

module.exports.v2 = function (args, done) {
  var seneca = this;
  seneca.act({role: 'cd-dojos', entity: 'lead', cmd: 'list', query: {}},
  function (err, leads) {
    if (err) return done(err);
    async.eachSeries(leads, function (lead, eCb) {
      async.waterfall([
        // Passing eCb is an obvious cheat. Sorry not sorry
        removeExtraField.bind(this, lead, eCb),
        save.bind(seneca)
      ]);
    }, done);
  });
...
function saveDojo (dojo, cb) {
  var seneca = this;
  seneca.act({role: 'cd-dojos', entity: 'dojo', cmd: 'save', dojo: dojo}, cb);
}

function migrateOldDojosVerificationDates (wfCb) {
  var seneca = this;
  seneca.act({role: 'cd-dojos', entity: 'dojo', cmd: 'list', query: {verified: 1, verifiedAt: null}},
  function (err, dojos) {
    async.eachSeries(dojos, function (dojo, eCb) {
      saveDojo.bind(seneca)({id: dojo.id, verifiedAt: dojo.created}, eCb);
    }, wfCb);
  });
}

function backfillCompletedAtDojoApplications (wfCb) {
  var seneca = this;
  seneca.act({role: 'cd-dojos', entity: 'lead', cmd: 'list', query: {completedAt: null, completed: true}},
  function (err, leads) {
    async.eachSeries(leads, function (lead, eCb) {
      save.bind(seneca)({id: lead.id, completedAt: lead.createdAt}, eCb);
    }, wfCb);
  });
}
...
module.exports = function (args, done) {
  async.waterfall([
    migrateOldDojosVerificationDates.bind(this),
    backfillCompletedAtDojoApplications.bind(this)
  ], done);
};
