 ...
 function sendSMSDataCollection (args, done) {
   ...
   var getChampions = function (waterfallCb) {
    ...
    function (err, dojos) {
      ...
      seneca.act({role: plugin, cmd: 'get_poll_results', query: {dojoId: {in$: _.map(dojos, 'id')},
      pollId: pollId}}, function (err, results) {
        if (err) return done(err);
        var dojoIds = _.map(results, 'dojoId');
        var unanswered = _.reject(dojos, function (dojo) {
          return dojoIds.indexOf(dojo.id) > -1;
        });
        if (unanswered.length > 0) {
          // Get champions
          seneca.act({role: plugin, cmd: 'load_usersdojos',
            query: {dojoId: {in$: _.map(unanswered, 'id')}}},
          function (err, users) {
            if (err) return done(err);
            // Remove
