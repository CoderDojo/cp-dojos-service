...
function saveSMSResult (args, done) {
  ...
  var getPoll = function (sms, wfCb) {
    ...
    seneca.act({role: plugin, cmd: 'get_poll_setup', query: {startedAt: {lt$: now}, endDate: {gt$: now}}}, function (err, polls) {
      if (err) return done(err);
      if (polls.length > 1) {
        return done(new Error('More than one poll at a time, notify a CDF'));
      }
      if (polls.length < 1) {
        return done(new Error('No poll running at that time'));
      }
  ...
  var getUserProfile = function (sms, poll, wfCb) {
    var phone = sms.From[0] === '+' ? sms.From.substr(1): sms.From;
     ...
     function (err, champProfiles) {
      ...
      seneca.act({role: plugin, cmd: 'load_usersdojos', query: {userId: {in$: _.map(champProfiles, 'userId')}}},
       function (err, usersDojos) {
        ...
        // if (champs.length > 1) {
        //   return done(new Error('Impossible to determinate unique champion from phone n° when saving poll answer'));
        // }
        // if (champs.length < 1) {
        //   return done(new Error('No champion found with phone n° when saving poll answer'));
        // }
  ...
  var getCheeringTwiML = function (champProfile, poll, waterfallCb) {
