...
function canAwardBadge (args, done) {
  ...
  if (userId && targetUserId) {
    seneca.make$(USER_DOJO_ENTITY_NS).native$(function (err, client, release) {
      if (err) {
        release();
        seneca.log.error(seneca.customValidatorLogger('cd-dojos', 'canAwardBadge::1', err, {userId: userId, applicationData: applicationData, targetUserId: targetUserId}));
      ...
      client.query('SELECT COUNT(*) FROM cd_usersdojos AS ud1 JOIN cd_usersdojos AS ud2 ON ud1.dojo_id=ud2.dojo_id'
        ...
        release();
        if (err) {
          seneca.log.error(seneca.customValidatorLogger('cd-dojos', 'canAwardBadge::2', err, {userId: userId, applicationData: applicationData, targetUserId: targetUserId}));
          return done(null, {'allowed': false});
        }
