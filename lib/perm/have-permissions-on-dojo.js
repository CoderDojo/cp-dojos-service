 ...
 */
function havePermissionsOnDojo (args, done) {
  var seneca = this;
  ...
  if (dojoId && userId) {
    seneca.make$(USER_DOJO_ENTITY_NS).load$({dojoId: dojoId, userId: userId, deleted: 0},  function (err, response) {
      if (err) {
        seneca.log.error(seneca.customValidatorLogFormatter('cd-dojos', 'havePermissions', err, {userId: userId, dojoId: dojoId}));
...
module.exports = havePermissionsOnDojo;
