...
function belongsToDojo (args, done) {
  ...
  seneca.make$(USER_DOJO_ENTITY_NS).list$({userId: userId, deleted: 0}, function (err, userDojos) {
    if (err) {
      seneca.log.error(seneca.customValidatorLogFormatter('cd-dojos', 'belongsTo::1', err, {userId: userId, dojoId: dojoId, targetUserId: targetUserId}));
    ...
    if (dojos.length > 0) {
      seneca.make$(USER_DOJO_ENTITY_NS).list$({userId: targetUserId, dojoId: { in$: dojos }, deleted: 0}, function (err, response) {
        if (err) {
          seneca.log.error(seneca.customValidatorLogFormatter('cd-dojos', 'belongsTo::2', err, {userId: userId, dojoId: dojoId, targetUserId: targetUserId, userDojos: userDojos}));
          return done(null, {'allowed': false});
        }
