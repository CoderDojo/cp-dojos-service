(function () {
  ...
  /**
   * Parse a csv to backfill phone numbers when those are acceptable
   * @param {String}  file   filePath
   * @return {[type]}        [description]
   * @example curl -d '{"role": "cd-dojos", "cmd":"backfill_champions", "file":"data/oldResponses.csv"}' http://localhost:10301/act
   */
      ...
      async.eachSeries(csvUsers, function(user, cb){
        ...
        if (correspondingZenUser) {
          ...
          if (country) {
            ...
            if (!isValid) {
              formatted = phoneFormatter.formatInternational(country, cleaned + "");
          ...
          correspondingZenUser.phone = correspondingZenUser.phone.replace('+', '');
